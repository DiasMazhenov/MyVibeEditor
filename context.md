...
- Git: локальный репозиторий и ветка `main` инициализированы, `origin` настроен на `https://github.com/DiasMazhenov/MyVibeEditor.git`; commit/push выполняются после этой синхронизации. Секретный `conf.ini` исключён через `.gitignore`.
