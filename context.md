...
## Текущее исправление v0.96

- Контекстное меню теперь содержит один пункт «Свойства»; отдельные HTML/CSS/media-действия удалены, чтобы выбранный объект всегда открывался в одном универсальном инспекторе.
- Инспектор разделён на пять секций: «Основные», «Ссылка», «Медиа», «CSS» и «Дополнительно». CSS, design tokens и внешний stylesheet остаются внутри общей CSS-секции.
- Любому безопасному объекту можно назначить ссылку: при отсутствии `<a>` создаётся безопасная обёртка с экранированными атрибутами; существующую ссылку можно изменить или удалить. Опасные URL и интерактивные вложения блокируются.
- Медиа-секция показывает текущие `src`, `alt`, `loading`, `decoding`, поддерживает live-синхронизацию и переиспользует существующий медиаменеджер для замены файла.
- Для inline SVG/иконок используется тот же target-контракт: при выборе контейнера с единственным медиа-потом инспектор показывает медиа-секцию; это сохраняет работу контекстного меню даже когда hit-test браузера попадает в оболочку.
- Дополнительные атрибуты ограничены безопасным allowlist (`data-*`, `name`, `value`, `placeholder`, `autocomplete`, `contenteditable`, `draggable`, `spellcheck`, `translate`) и синхронизируются через общий helper `syncMarkupAttributes`.
- Добавлены общие responsive-grid/action/status стили в theme и fallback; desktop сохраняет правый inspector, mobile — нижнюю прокручиваемую панель.
- Контрактные проверки: `node --check myvibehtml.js` и 17 feature-contract тестов проходят.
- Live-проверка Playwright на локальном `127.0.0.1:8080`: авторизация → right-click → «Свойства» → проверка пяти секций → ввод `font-size=42` (computed `42px`) → `data-live-check=properties-v096` в DOM/source → ссылка `#properties-live-check` в DOM/source → mobile 390px (`44px`, right `378`) без ошибок. Скриншоты: `/private/tmp/myvibe-properties-menu-v096.png`, `/private/tmp/myvibe-properties-inspector-v096.png`.
- Версия runtime/cache-busting повышена до `0.96`; пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в commit.
...
- Поля размеров, ширины, высоты, gap, font-size, line-height и border-radius используют `input type="number"` с сохранением текущей CSS-единицы; поля `color` и `background-color` показывают hex и имеют нативный `input type="color"`.
- Сетки CSS/HTML-инспектора переведены на две колонки: display и тег занимают полную строку, ID/классы и остальные пары не выезжают за границы панели; fallback и theme синхронизированы.
- Версия `0.84` синхронизирована в runtime, заголовках модулей, cache-busting, документации, тестах и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в commit.
- Проверки прошли: PHP lint, JS syntax, 30 Node-тестов, CI contract, security smoke, `git diff --check` и полный HTTP regression против `127.0.0.1:8080`.
- Live HTTP подтвердил: editor без сессии возвращает ожидаемый `403`, `myvibehtml.js`, source-map и оба CSS-слоя отдаются с `200`. URL `?q=demo-about.html&rev=0.84` открыт во встроенном браузере.
- В доступном API Codex нет программного управления вкладкой встроенного браузера (click/fill/screenshot): поэтому ручное заполнение чисел и hex в самом уже авторизованном окне не выдаётся за выполненное. Сценарий ввода добавлен в `docs/browser-acceptance.md`; HTTP/static проверки не заменяют этот ручной шаг.
- Следующие этапы: v0.85 — каталог контента; v0.86 — массовые операции; v0.87 — расширенный медиаменеджер.

## Текущее исправление v0.83

- Исправлен корень проблемы CSS-инспектора: CSS, HTML-атрибуты и сброс style используют диапазон открывающего тега; структурные операции сохранены на полном диапазоне элемента.
- Скроллбары редактора, CSS-инспектора и отдельной админки получили фирменную graphite/teal-палитру: `scrollbar-color` для Firefox и `::-webkit-scrollbar` для Chromium/WebKit.
- Версия `0.83` синхронизирована в runtime, заголовках модулей, cache-busting, документации, тестах и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в commit.
- Проверки v0.83 прошли: PHP lint, JS syntax, 30 Node-тестов, CI contract, security smoke, `git diff --check` и полный HTTP regression против `127.0.0.1:8080`.
- Live URL `?q=demo-about.html&rev=0.83` открыт во встроенном браузере; программного click/screenshot API в текущем приложении нет, поэтому проверка подтверждает загрузку live-страницы и HTTP-ресурсов, но не выдаёт ручной клик за автоматический.
- Следующие этапы: v0.84 — каталог контента; v0.85 — массовые операции; v0.86 — расширенный медиаменеджер.

## Текущее исправление v0.82

- Из HTML-инспектора удалены поля `role`, `aria-label` и `aria-hidden`; заголовок fieldset и пункт контекстного меню теперь называются `HTML`. ARIA-атрибуты интерфейса редактора и Page Health сохранены.
- Исправлен CSS-инспектор: после валидного изменения свойства обновляются live-DOM, исходник, local draft/timeline и dirty-state, поэтому кнопка `Сохранить` активируется и изменение не теряется.
- Версия `0.82` синхронизирована в runtime, заголовках модулей, cache-busting, документации, тестах и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в commit.
- Проверки v0.82 прошли: PHP lint, JS syntax, 29 Node-тестов, CI contract, security smoke, `git diff --check` и полный HTTP regression против `127.0.0.1:8080`. Live URL `?q=demo-about.html&rev=0.82` открыт во встроенном браузере; программного click/screenshot API в текущем приложении нет, поэтому ручной клик по инспектору не выдаётся за автоматическую проверку.
- Следующие этапы: v0.83 — каталог контента; v0.84 — массовые операции; v0.85 — расширенный медиаменеджер.

## Текущее исправление v0.81

- В Pages и динамическом Files-обозревателе у файлов появилась локальная `file.svg`-иконка из Tabler/Iconify; папки используют `folder.svg`.
- Иконки папок и файлов занимают одинаковый 20-px слот, поэтому названия остаются выровнены по одной вертикали.
- Версия `0.81` синхронизирована в runtime, заголовках модулей, cache-busting, документации, тестах и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в коммит.
- После изменений проверить PHP/JS syntax, Node/feature/CI/security/regression и live `?admin=1#pages`; визуально убедиться в наличии file/folder icons и общей линии текста.
- Следующие этапы: v0.82 — каталог контента; v0.83 — массовые операции; v0.84 — расширенный медиаменеджер.

## Текущее исправление v0.80

- В админском Pages/Files-обозревателе папки показываются локальной иконкой `folder.svg` рядом с названием; подпись `Folder` удалена.
- Таблица Pages стала dropzone: drag-and-drop принимает один или несколько файлов и загружает их в текущую папку через существующий CSRF upload endpoint. Поле выбора файла визуально скрыто и используется только кнопкой Upload.
- Кнопки Pages toolbar заменяются локальными Tabler/Iconify-иконками `arrow-up`, `copy`, `clipboard`, `copy-plus`, `pencil`, `trash`, `upload`, `folder-plus`; исходные подписи сохраняются в `title` и `aria-label`.
- Названия папок и файлов получили единый слот иконки и горизонтальное выравнивание; специфичный selector убирает рамку у folder-button, а подписи `Folder` и `HTML` в Pages не рендерятся.
- Версия `0.80` синхронизирована в runtime, заголовках модулей, cache-busting, документации, тестах и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в коммит.
- После изменений проверить PHP/JS syntax, Node/feature/CI/security/regression, dropzone static contract и live `?admin=1#pages`; визуально убедиться, что у папок есть иконка, `Folder` не виден, toolbar icon-only, а drag-over подсвечивает таблицу.
- Следующие этапы: v0.81 — каталог контента; v0.82 — массовые операции; v0.83 — расширенный медиаменеджер.

## Текущее исправление v0.79

- Раздел «Страницы» админки стартует в папке текущего default-файла, показывает только HTML/XHTML-файлы и каталоги для навигации, а имена HTML-файлов ведут в редактор выбранной страницы.
- Над таблицей добавлен toolbar «Вверх», «Копировать», «Вставить», «Дублировать», «Переименовать», «Удалить», Upload и New folder. JS хранит отдельное состояние `pageState`, сбрасывает выбор после операции и не смешивает его с файловым обозревателем.
- Backend `duplicate` теперь принимает `path` как целевую открытую папку, поэтому paste между каталогами действительно вставляет копию в текущую папку; исходный `source` и target по-прежнему проходят нормализацию безопасного пути.
- Авторизация центрируется через локальные override-правила `#a{display:grid;place-items:center;min-height:100dvh}` в theme и fallback, сохраняя существующую responsive-карточку.
- Версия `0.79` синхронизирована в runtime, модульных заголовках, cache-busting, README, changelog, architecture, accessibility, browser acceptance и UI snapshot. Пользовательские `test-page.html` и `.test-page.html.myvibehtml.lock` не входят в коммит.
- Проверки пройдены: PHP/JS syntax checks, 28 Node-тестов, `tests/ci-contract.sh`, `security-smoke.sh`, полный HTTP `tests/regression.sh`, `git diff --check` и авторизованный admin-pages API smoke с текущей папкой `myvibe` и demo-страницами. Встроенный браузер нужно открыть на `?admin=1#pages` для финальной визуальной сверки.
- Следующие этапы: v0.80 — каталог контента; v0.81 — массовые операции; v0.82 — расширенный медиаменеджер.
