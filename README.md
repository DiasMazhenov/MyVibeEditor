...
- локальная библиотека переиспользуемых компонентов и медиаменеджер изображений;
...
Кнопка компонентов открывает локальную библиотеку в браузере. В контекстном меню выбранного узла доступно `Сохранить компонент`; сохранённый компонент можно вставить после текущего выделения, обновить текущим выделением или удалить. Старые записи библиотеки автоматически получают совместимый id при чтении. Кнопка `Медиа` в файловом менеджере показывает найденные изображения (`avif`, `gif`, `ico`, `jpg`, `jpeg`, `png`, `svg`, `webp`) и открывает их прямой URL.
