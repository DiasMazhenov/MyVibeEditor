...
Проверка перед сохранением дополнительно ищет отсутствие `lang`, пустой/отсутствующий `title`, `meta description`, `meta viewport`, неверное количество `h1`, ссылки без `href` и чрезмерно тяжёлую страницу по числу ресурсов/строк. Проверка локальная: страница и код никуда не отправляются.
