...
- контекстное меню упрощено до единого действия «Свойства» вместо раздельных HTML/CSS/media-пунктов;
- добавлен универсальный инспектор из пяти секций: Основные, Ссылка, Медиа, CSS и Дополнительно;
- любой безопасный объект можно обернуть ссылкой, а существующую ссылку — изменить или удалить без выхода из инспектора;
- добавлены текущие media-поля и безопасные пользовательские атрибуты с синхронизацией в preview, draft и исходник;
- добавлен каталог контента админки с фильтрами по типам и accessibility-проблемам;
- добавлены массовые файловые операции для файлов и папок: copy, move, archive, download и rollback;
- добавлен медиаменеджер с предпросмотром, MIME, размерами, alt-статусом, заменой и before/after diff;
- контекстные действия CSS объединены в один пункт «Изменить CSS»: встроенные и внешние правила открываются в одном универсальном инспекторе;
- версия и cache-busting синхронизированы до `0.96`.
...
- Добавлены live-preview, media-контекст, diff и защищённое `save_css`-сохранение с backup и атомарной записью.
- Live-проверка подтверждает: изменение `box-shadow` переживает `Save` и reload; последующее восстановление также переживает reload.

## 0.88

- CSS-инспектор теперь сохраняет изменённый HTML: dirty-state остаётся активным до успешного ответа сервера, а save-запрос всегда получает token и полный Base64-источник.
- Playwright live-проверка подтверждает `right-click → Изменить CSS → ввод 42 → Сохранить → reload`: POST возвращает `200`, а `font-size: 42px` сохраняется.

## 0.87

- Исправлена причина неработающего «Изменить CSS»: source map теперь сопоставляет body-фрагмент исходника с синтетическими `html/head`-узлами preview.
- Реальная проверка Playwright подтверждает цепочку `right-click → Изменить CSS → ввод 42 → font-size: 42px` в live DOM и HTML draft.

## 0.86

- Контекстное меню теперь использует явный `data-context-action`, останавливает всплытие клика и обрабатывает действие через фактически нажатую кнопку. Пункт «Изменить CSS» снова открывает инспектор выбранного элемента, не теряя target из-за document-level dismiss handler.
- Добавлен feature-contract для цепочки `context menu → CSS inspector`.
