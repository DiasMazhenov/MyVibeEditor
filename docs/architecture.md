...
v0.96 сохраняет безопасные модульные границы без bundler/dependency, размещает единый инспектор «Свойства» справа на desktop, выносит auth-flow, shell-контролы и transport-примитивы в отдельные модули. Инспектор состоит из пяти секций: базовая разметка, ссылки, медиа, CSS (включая внешний stylesheet и design tokens) и безопасные пользовательские атрибуты. Ссылки назначаются объектам через экранированную `<a>`-обёртку только для совместимых узлов; опасные URL и интерактивные вложения отклоняются. CSS и HTML-атрибуты синхронизируются по диапазону открывающего тега, а структурные операции используют полный диапазон узла. Source map учитывает body-фрагмент исходника и синтетические `html/head`-узлы preview, поэтому live CSS получает корректный диапазон. Любое изменение через инспектор помечает visual draft dirty, сохраняет его до успешного POST и не отключает кнопку `Сохранить`, пока сервер не подтвердит запись. Save всегда создаёт token и отправляет полный Base64-источник. Все preview-only узлы, включая служебные `<base>`, `<style>` и `<edit>`, получают `data-myvibehtml-preview="true"` и исключаются из source map, чтобы live-изменения не становились неоднозначными после инициализации редактора. Контекстное меню передаёт действие через `data-context-action` и обрабатывает клик через `event.currentTarget`, поэтому document-level dismiss не перехватывает «Свойства». Numeric CSS-поля сохраняют исходные единицы, цвета редактируются как hex через нативный color picker. Scrollbar использует фирменные graphite/teal-цвета. CSP enforcing включён для auth/source/error, а visual preview переключается в enforcement только явным `MYVIBEHTML_CSP_VISUAL_ENFORCE=1`:
