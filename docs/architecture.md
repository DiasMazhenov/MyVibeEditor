...
v0.87 сохраняет безопасные модульные границы без bundler/dependency, размещает CSS-инспектор справа на desktop, выносит auth-flow, shell-контролы и transport-примитивы в отдельные модули. Инспектор редактирует HTML-идентификаторы и CSS, но не ARIA-поля; CSS и HTML-атрибуты синхронизируются по диапазону открывающего тега, а структурные операции используют полный диапазон узла. Source map учитывает body-фрагмент исходника и синтетические `html/head`-узлы preview, поэтому live CSS получает корректный диапазон. Все preview-only узлы, включая служебные `<base>`, `<style>` и `<edit>`, получают `data-myvibehtml-preview="true"` и исключаются из source map, чтобы live-изменения не становились неоднозначными после инициализации редактора. Контекстное меню передаёт действие через `data-context-action` и обрабатывает клик через `event.currentTarget`, поэтому document-level dismiss не перехватывает «Изменить CSS». Numeric CSS-поля сохраняют исходные единицы, цвета редактируются как hex через нативный color picker. Scrollbar использует фирменные graphite/teal-цвета. CSP enforcing включён для auth/source/error, а visual preview переключается в enforcement только явным `MYVIBEHTML_CSP_VISUAL_ENFORCE=1`:
